...
export default function BecomeSaller() {
  ...
  const browseLogoImg = () => {
    (logoImgInput as any).current.click();
  };
  const logoImgChangHandler = (e: any) => {
    if (e.target.value !== "") {
      const imgReader = new FileReader();
      imgReader.onload = (event) => {
        setLogoImg(((event.target as any).result) as any);
      };
  ...
  const browseProfileImg = () => {
    (profileImgInput as any).current.click();
  };
  const profileImgChangHandler = (e: any) => {
    if (e.target.value !== "") {
      const imgReader = new FileReader();
      imgReader.onload = (event) => {
        setProfileImg((event.target as any).result);
      };
      imgReader.readAsDataURL(e.target.files[0]);
    }
  };
  ...
  const browseCoverImg = () => {
    (coverImgInput.current as any).click();
  };
  const coverImgChangHandler = (e: any) => {
    if (e.target.value !== "") {
      const imgReader = new FileReader();
      imgReader.onload = (event) => {
        setCoverImg((event.target as any).result);
      };
      imgReader.readAsDataURL(e.target.files[0]);
    }
  };
