...
export interface IWishlistTab {
  [x: string]: any;
}

export default function WishlistTab({ className }: IWishlistTab) {
        ...
        <div className="relative w-full overflow-x-auto border border-[#EDEDED]">
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <tbody>
              ...
              {/* table heading end */}
              {wishlist.map((w: ICart, index: number) => {
                return (
                    ...
                    <td className="pl-10  py-4 ">
                      <div className="flex space-x-6 items-center">
                        <div className="w-[80px] h-[80px] overflow-hidden flex justify-center items-center border border-[#EDEDED]">
                          <img
                            src={`${process.env.PUBLIC_URL}/${w.image}`}
                            alt="product"
            ...
            <button
              ...
              onClick={() => {
                ...
                for (const wish of wishlist) {
                  const itemInCart = clonedCart.find((c) => c.id === wish.id);
                  if (itemInCart) {
