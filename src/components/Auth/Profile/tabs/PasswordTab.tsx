  ...
  const { fields, submit, dirty, submitting } = useForm({
    fields: {
      ...
      newPassword: useField({
        ...
        validates: (value) => {
          ...
          if (value.length < 6) return 'Password is too short';
          if (value.length > 50) return 'Password is too long';
          if (value === fields.oldPassword.value)
            return 'New password must be different from old password';
          if (value === fields.newPassword.value)
            fields.confirmPassword.setError('');
          if (value !== fields.confirmPassword.value)
            fields.confirmPassword.onBlur();
        },
      }),
    ...
    onSubmit: async (fieldValues) => {
      const response = await fetch(
        `${baseUrl}/users/${user!.id}/update-password`,
        {
          method: 'PUT',
          headers: {
      ...
      if (response.message) {
        ...
        fields.oldPassword.reset();
        fields.newPassword.reset();
          ...
          <div className="input-field mb-6">
            ...
            <div className="input-wrapper border border-[#E8E8E8] w-full  h-[58px] overflow-hidden relative ">
              <input
                ...
                onChange={fields.oldPassword.onChange}
                onBlur={fields.oldPassword.onBlur}
              />
              <div
            ...
            {fields.oldPassword.error && (
              <span style={{ color: 'red' }}>{fields.oldPassword.error}</span>
            )}
          </div>
          <div className="input-field mb-6">
            <label
            ...
            <div className="input-wrapper border border-[#E8E8E8] w-full  h-[58px] overflow-hidden relative ">
              <input
                ...
                onChange={fields.newPassword.onChange}
            ...
            {fields.newPassword.error && (
              <span style={{ color: 'red' }}>{fields.newPassword.error}</span>
            )}
          </div>
          <div className="input-field mb-6">
            <label
