...
import { formatFeatureKey } from '../../utils/util';
...
export default function SingleProductPage() {
  ...
  const fetchProduct = async (slug: string) => {
    const response = await fetch(`${baseUrl}/products/${slug}`).then(
      (response) => response.json(),
    );
    setProduct(response);
    const productsResponse = await fetch(
      `${baseUrl}/products/related-products/${response.type}/${slug}`,
    ).then((response) => response.json());
    setRelatedProducts(productsResponse);
  ...
  const reviewAction = () => {
    ...
    setTimeout(() => {
      if (rating) {
        ...
        setHover(0);
        window.scrollTo({
          top: (-reviewElement.current! as any).getBoundingClientRect().top,
          left: 0,
          behavior: 'smooth',
        });
      }
            ...
            <div className="breadcrumb-wrapper w-full ">
              <div className="container-x mx-auto">
                <BreadcrumbCom
                  paths={[
                    { name: 'home', path: '/' },
                    { name: params.id, path: `/products/${params.id}` },
                  ]}
                />
              </div>
            </div>
                  ...
                  <div data-aos="fade-up" className="w-full tab-content-item">
                    ...
                    <div>
                      ...
                      <ul className="list-disc ml-[15px]">
                        {product?.attributes ? (
                          Object.entries(JSON.parse(product!.attributes)).map(
                            ([k, v]) => {
                              return (
                                <li className="font-normal text-qgray leading-9">
                                  {formatFeatureKey(k)}: {v as any}
                                </li>
                              );
                            },
                          )
                ...
                {tab === 'review' && (
                  <div data-aos="fade-up" className="w-full tab-content-item">
                    ...
                    <div className="w-full">
                      {product?.reviews &&
                      JSON.parse(product?.reviews).length ? (
                        <Reviews
